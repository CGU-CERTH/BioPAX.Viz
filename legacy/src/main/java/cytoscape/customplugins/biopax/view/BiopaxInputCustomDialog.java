package cytoscape.customplugins.biopax.view;
...
public class BiopaxInputCustomDialog extends JDialog implements ActionListener {
	...
	public BiopaxInputCustomDialog(Frame aOwner) {
		super(aOwner, "BioPAX Viz - Select file(s)", true);
		...
		if (src == btnInputFile && (SELECT_DIR == false)) {
			...
			if (file != null) {
				if (checkInputFile(file)) {
					...
					Utils.showErrorBox("Selected input file is not acceptable.", "Please select a '.owl' file.");
				}
				updateStartButton();
			}
				...
				if (checkInputDir(file)) {
                                ...
                                if(validDir){
                                    ...
                                    txfInputFile.setText("");
                                    Utils.showErrorBox("Selected input directory is not acceptable.", "Please select a directory containing only '.owl' files and a '.nodes' file.");
                                }
